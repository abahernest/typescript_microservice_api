...
describe('Test Inbound SMS', () => {
    describe('Checking HTTP Request Type', () => {
        test('Should return 405 for HTTP GET request', async () => {
            const result = await request(app).get('/api/v1/inbound/sms').send(inboundRequest);
        ...
        test('Should return 405 for HTTP DELETE request', async () => {
            const result = await request(app).delete('/api/v1/inbound/sms').send(inboundRequest);
        ...
        test('Should return 405 for HTTP PATCH request', async () => {
            const result = await request(app).patch('/api/v1/inbound/sms').send(inboundRequest);
        ...
        test('Should return 405 for HTTP PUT request', async () => {
            const result = await request(app).put('/api/v1/inbound/sms').send(inboundRequest);
    ...
    describe('Account Authorization', () => {
        test('Should return 403 for wrong account credentials', async () => {
            const result = await request(app).post('/api/v1/inbound/sms').auth("abcdef","abcdef").send(inboundRequest);
        ...
        test('Should return 403 for empty account credentials', async () => {
            const result = await request(app).post('/api/v1/inbound/sms').send(inboundRequest)
        ...
        test('Should return 200 for correct account credentials', async () => {
            const result = await request(app).post('/api/v1/inbound/sms').auth(account.username,account.auth_id).send(inboundRequest);
    ...
    describe('Request Body Validation', () => {
        ...
        test('Should return "inbound sms ok" if request validation passes', async () => {
            const result = await request(app)
            .post('/api/v1/inbound/sms')
            .auth(account.username,account.auth_id)
            .send(inboundRequest);
            ...
            expect(result.status).toEqual(200)
            expect(result.body.message).toEqual("inbound sms ok");
        })

    })

    describe('Phone Number Verification', () => {
        test('Should return "to parameter not found" when phone number doesnt exist', async () => {
            const result = await request(app)
            .post('/api/v1/inbound/sms')
            .auth(account.username,account.auth_id)
            .send({
            ...
            expect(result.body.error).toEqual("to parameter not found");
        })

        test('Should return "inbound sms ok" if phone number is found', async () => {
            const result = await request(app)
            .post('/api/v1/inbound/sms')
            .auth(account.username,account.auth_id)
            .send(inboundRequest);
